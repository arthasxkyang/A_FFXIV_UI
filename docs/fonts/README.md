...
未映射合计 1,923 个，使用衍生字体时仍可能显示缺字符号。此方案不能称为完整中文支持。字体加载成功也不意味着所有文本均有字形，客户端回退仅处理字体文件加载失败，不提供逐字符回退。
...
5. 允许 [reviewed-variants.json](../../tools/font-data/reviewed-variants.json) 中逐项审核的 53 条新增规则。直接检查日式异体字，也允许经过唯一繁体目标的一跳异体关系；构建会核对每一条证据路径。歧义简繁条目仍不可被白名单覆盖。
6. `kSemanticVariant` 只用于产生候选，需逐字判断；`kSpecializedSemanticVariant` 的局部词义/古籍通假候选禁止直接填入。例如“你→祢／袮”“她→他”不能作为现代中文显示的替代字。
7. 其他缺字列入 `unavailable`，意为“现有规则尚无获准映射”，不等于已证明源字体绝无可用字形。

本轮检查了固定版本 OpenCC、Unicode 17.0.0 Unihan 异体关系，以及 Adobe `UniJIS-UTF32-H` 到源字体 Adobe-Japan1-3 CID 的映射。剩余 1,923 字中，178 字检索到待审核候选、1,745 字在本次数据和检索路径中未找到候选；Adobe 标准横排 CMap 没有为这些剩余字符指向源字体中存在的同字字形。该结果不是对所有历史异体关系或未收录字形的穷尽证明。

完整映射、歧义清单、尚未补齐清单及 SHA-256 见 [axis-coverage.json](axis-coverage.json)。每行 30 字的最新缺字表见 [axis-missing-30.txt](axis-missing-30.txt)，新增映射审核见 [本轮审核表](axis-variant-review.md)。字典来源和许可见 [数据说明](../../tools/font-data/README.md)。源字体更换时构建会因哈希不符而中止，必须重新审核。
...
构建产物为 `Media/Fonts/AxisRegular-CJKReuse.ttf`，构建同时更新覆盖报告和每行 30 字的缺字表；重复构建应生成相同字节。预览输出为 `docs/fonts/axis-preview.png`，该图片是离线 FreeType 渲染，不是游戏截图。

测试覆盖原始映射和未修改字体表、所有新增字形的像素及字宽一致性、歧义排除、覆盖统计、可重复构建、Lua 5.1 语法（忽略源文件开头的 UTF-8 BOM）、语言选择和文件加载失败回退，以及七处文本接入和 TOC 顺序。本轮另验证上一轮 1,402 条映射不变、53 条增量规则生效、伪造证据及歧义绕过被拒绝、候选审计统计和每行 30 字输出；共 12 项测试。
